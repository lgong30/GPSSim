...
%% virtual time axis
\draw[thick,->] (0,0) -- ({{ width }},0) node[anchor=north west] {virtual time};
\draw[thick,->] (0,0) -- (0,{{ height }}) node[anchor=south east] {flow};
...
\filldraw[fill=white, draw=blue,very thick] ({{ pkt.v_start_time }}, {{ pkt.y_min }} ) rectangle ({{ pkt.v_finish_time }}, {{ pkt.y_max }});
{% endfor %}
{% endfor %}
